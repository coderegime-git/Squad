...
class ClubAdminScheduleScreen extends StatefulWidget {
  ...
  @override
  State<ClubAdminScheduleScreen> createState() => _ClubAdminScheduleScreenState();
}
...
class _ClubAdminScheduleScreenState extends State<ClubAdminScheduleScreen> {
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: Scaffold(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      // Filter removed per client request
                    ],
                  ),
                ),
              ),
              ...
              child: _loadingEvents
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      decoration: BoxDecoration(
                        color: cardDark, borderRadius: BorderRadius.circular(22.r),
                      ...
                      child: TableCalendar<Data>(
                        ...
                        calendarStyle: CalendarStyle(
                          todayDecoration: BoxDecoration(color: accentGreen.withOpacity(0.3), shape: BoxShape.circle),
                          selectedDecoration: const BoxDecoration(color: accentGreen, shape: BoxShape.circle),
                          markerDecoration: BoxDecoration(color: accentOrange, shape: BoxShape.circle),
                          weekendTextStyle: TextStyle(color: accentOrange),
                        ),
                        headerStyle: HeaderStyle(
                          formatButtonVisible: true, titleCentered: true,
                          formatButtonDecoration: BoxDecoration(color: accentGreen.withOpacity(0.15), borderRadius: BorderRadius.circular(10.r)),
                          formatButtonTextStyle: TextStyle(color: accentGreen),
                          titleTextStyle: GoogleFonts.montserrat(fontSize: 16.sp, fontWeight: FontWeight.bold),
                          leftChevronIcon: Icon(Icons.chevron_left_rounded, color: textSecondary),
                          rightChevronIcon: Icon(Icons.chevron_right_rounded, color: textSecondary),
                        ),
                      ),
                    ),
                    20.height,
                    Row(
                      ...
                      children: [
                        Text(DateFormat('EEE, MMM d').format(_selectedDay ?? DateTime.now()),
                    ...
                    dayEvents.isEmpty
                      ...
                      decoration: BoxDecoration(
                        color: cardDark, borderRadius: BorderRadius.circular(18.r),
                        border: Border.all(color: Colors.grey.shade300),
                      ),
                      child: Column(children: [
                        Icon(Icons.event_busy_rounded, size: 48.sp, color: textSecondary),
                        12.height,
                        Text('No events scheduled', style: GoogleFonts.poppins(color: textSecondary)),
                      ]),
                    )
                        : Column(children: dayEvents.map((e) => Padding(
        ...
        floatingActionButton: FloatingActionButton.extended(
          ...
          label: Text('Create Event', style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.w600)),
        ),
      ),
    );
  }
  ...
  Widget _eventCard(Data e) {
    final color = e.eventType == 'TRAINING' ? accentGreen : e.eventType == 'MATCH' ? accentOrange : Colors.purple;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: cardDark, borderRadius: BorderRadius.circular(20.r),
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Container(
                  ...
                  child: Text(e.eventType,
                      style: GoogleFonts.poppins(fontSize: 10.sp, color: color, fontWeight: FontWeight.w700, letterSpacing: 0.8)),
                ),
                const Spacer(),
                // Three-dot menu: ONLY Cancel Event (per client request)
                PopupMenuButton(
                  color: Colors.grey.shade200,
                  icon: Icon(Icons.more_vert_rounded, color: textSecondary),
                  itemBuilder: (_) => [
                    PopupMenuItem(
                      child: Text('Cancel Event', style: GoogleFonts.poppins(color: Colors.red)),
            ...
            10.height,
            Text(e.eventName,
                style: GoogleFonts.montserrat(fontSize: 15.sp, fontWeight: FontWeight.w700, color: Colors.black)),
            8.height,
            ...
            Row(
              children: [
                ...
                6.width,
                Text('${e.coachIds.length} coach${e.coachIds.length != 1 ? 'es' : ''}',
                    style: GoogleFonts.poppins(fontSize: 12.sp, color: textSecondary)),
                const Spacer(),
                Text(e.status, style: GoogleFonts.poppins(fontSize: 11.sp, color: color, fontWeight: FontWeight.w600)),
              ],
            ),
          ],
        ),
  ...
  Widget _eRow(IconData icon, String text) => Row(children: [
    ...
    6.width,
    Text(text, style: GoogleFonts.poppins(fontSize: 12.sp, color: textSecondary)),
  ]);
  ...
  void _confirmDelete(Data e) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text('Cancel Event', style: GoogleFonts.montserrat(color: Colors.black)),
        content: Text('Are you sure you want to cancel "${e.eventName}"?',
            style: GoogleFonts.poppins(color: textSecondary)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context),
              child: Text('Back', style: GoogleFonts.poppins(color: textSecondary))),
          TextButton(
            onPressed: () async {
            ...
            child: Text('Cancel Event', style: GoogleFonts.poppins(color: Colors.red, fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
  ...
  void _showEventDetailSheet(Data event) {
    showModalBottomSheet(
      ...
      builder: (_) => EventDetailSheet(eventId: event.eventId, event: event, apiService: _apiService),
    );
  }
...
// ── Event Detail Sheet — "Invite Groups" button ──────────────────────────────
class EventDetailSheet extends StatefulWidget {
  final int eventId;
  final Data event;
  final ClubApiService apiService;

  const EventDetailSheet({required this.eventId, required this.event, required this.apiService});
...
class EventDetailSheetState extends State<EventDetailSheet> {
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() { _event = result.data; _loading = false; });
    } catch (e) {
      setState(() { _error = 'Failed to load event details.'; _loading = false; });
    }
  }
  ...
  Color _typeColor(String type) => type == 'TRAINING' ? accentGreen : type == 'MATCH' ? accentOrange : Colors.purple;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: _loading
          ? SizedBox(height: 180.h, child: const Center(child: CircularProgressIndicator(color: accentGreen)))
          : _error != null
          ? SizedBox(height: 180.h, child: Center(child: Text(_error!, style: GoogleFonts.poppins(color: textSecondary))))
          : Column(
        ...
        children: [
          ...
          Text(_event!.eventName,
              style: GoogleFonts.montserrat(fontSize: 18.sp, fontWeight: FontWeight.bold, color: Colors.black)),
          16.height,
          _detailRow(Icons.calendar_today_rounded, 'Date', _event!.eventDate),
          10.height,
          _detailRow(Icons.access_time_rounded, 'Time', '${_formatTime(_event!.startTime)} – ${_formatTime(_event!.endTime)}'),
          10.height,
          _detailRow(Icons.location_on_outlined, 'Location', _event!.location),
          10.height,
          _detailRow(Icons.person_rounded, 'Created by', _event!.createdByUsername),
          10.height,
          _detailRow(Icons.group_rounded, 'Coaches', '${_event!.coachIds.length} assigned'),
          20.height,

          // "Invite Groups" button (renamed from "Manage Groups" per client)
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: () {
                Navigator.pop(context);
                Navigator.push(context, MaterialPageRoute(builder: (_) => InviteGroupsScreen(event: widget.event)));
              },
              icon: Icon(Icons.group_add_rounded, size: 20.sp),
              label: Text('Invite Groups', style: GoogleFonts.poppins(fontSize: 14.sp, fontWeight: FontWeight.w700)),
  ...
  Widget _badge(String label, Color color) => Container(
    padding: EdgeInsets.symmetric(horizontal: 10.w, vertical: 4.h),
    decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(20.r)),
    child: Text(label, style: GoogleFonts.poppins(fontSize: 11.sp, color: color, fontWeight: FontWeight.w700)),
  );
  ...
  Widget _detailRow(IconData icon, String label, String value) => Row(
    ...
    children: [
      ...
      8.width,
      Text('$label: ', style: GoogleFonts.poppins(fontSize: 13.sp, fontWeight: FontWeight.w600, color: Colors.black87)),
      Expanded(child: Text(value, style: GoogleFonts.poppins(fontSize: 13.sp, color: textSecondary))),
    ],
  );
}
...
class _CreateEventSheetState extends State<_CreateEventSheet> {
  ...
  // Event type simplified: Single Event or Recurring only (no match/tournament separation per client)
  final List<String> _eventTypes = ['SINGLE_EVENT'];
